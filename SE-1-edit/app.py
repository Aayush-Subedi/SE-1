...
def agent():
    if request.method == "POST":
        ...
        cursor = con.cursor()
        query = "SELECT username, password FROM Agent WHERE username = '{u}' AND password = '{p}'".format(u=username,p=password) # Checking username and password with database
...
def hospital():
    if request.method == "POST":
        ...
        con=sqlite3.connect("database.db") # connection with database
        cursor = con.cursor()
        query = "SELECT username, password FROM Hospital WHERE username = '{u}' AND password = '{p}'".format(u=username,p=password) # Same logic as agent login 
        rows = cursor.execute(query)
        rows = rows.fetchall()
        if(len(rows) >= 1):
            return render_template('success.html')
